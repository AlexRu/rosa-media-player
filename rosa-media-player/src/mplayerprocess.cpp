...
void MplayerProcess::goToPos( double perc )
{
    qDebug( "MplayerProcess::goToPos: per: %f", perc );

    tellcmd( "seek " + QString::number( perc ) + " 1" );
}

void MplayerProcess::goToSec( double sec )
{
    qDebug( "MplayerProcess::goToSec: %f", sec );

    tellcmd( "seek " + QString::number( sec ) + " 2" );
}

void MplayerProcess::seek( int secs )
{
    qDebug( "MplayerProcess::seek: %d", secs );

    tellcmd( "seek " + QString::number( secs ) + " 0" );
}

void MplayerProcess::setVolume( int volume )
{
    tellcmd( "volume " + QString::number( volume ) + " 1" );
}

void MplayerProcess::mute( bool b )
{
    int v = ( b ? 1 : 0 );
    tellcmd( pausing_prefix() + " mute " + QString::number( v ) );
}

QString MplayerProcess::pausing_prefix()
{
    qDebug("MplayerProcess::pausing_prefix");
...
void MplayerProcess::tellcmd( const QString &command )
{
    qDebug("MplayerProcess::tellcmd: '%s'", command.toUtf8().data() );
    ...
    if ( MplayerProcess::isRunning() ) {
        ...
        qWarning( " tellcmd: no process running: %s", command.toUtf8().data() );
    }
}

void MplayerProcess::initFormats()
{
    qDebug() << "MplayerProcess::initFormats";
