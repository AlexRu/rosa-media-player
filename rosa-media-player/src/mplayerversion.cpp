...
int MplayerVersion::mplayerVersion(QString string)
{
    ...
    else if (rx_mplayer_revision.indexIn(string) > -1)
    {
        mplayer_svn = rx_mplayer_revision.cap(2).toInt();
        qDebug("MplayerVersion::mplayerVersion: MPlayer SVN revision found: %d", mplayer_svn);
    }
    else if (rx_mplayer_version.indexIn(string) > -1)
    {
        QString version = rx_mplayer_version.cap(1);
        qDebug("MplayerVersion::mplayerVersion: MPlayer version found: %s", version.toUtf8().data());
        mplayer_svn = 0;
        if (version == "1.0rc3" || version == "1.1") mplayer_svn = MPLAYER_1_0_RC3_SVN;
        else if (version == "1.0rc2") mplayer_svn = MPLAYER_1_0_RC2_SVN;
        else if (version == "1.0rc1") mplayer_svn = MPLAYER_1_0_RC1_SVN;
...
bool MplayerVersion::isMplayerAtLeast(int svn_revision)
{
    ...
    else if (pref->mplayer_user_supplied_version != -1)
    {
...
QString MplayerVersion::toString(int svn_revision)
{
    ...
    switch (svn_revision)
    {
    case MPLAYER_1_0_RC1_SVN:
        version = QString("1.0rc1");
        break;
    case MPLAYER_1_0_RC2_SVN:
        version = QString("1.0rc2");
        break;
    case MPLAYER_1_0_RC3_SVN:
        version = QString("1.0rc3");
        break;
    default :
        version =  QString("SVN r%1").arg(svn_revision);
    }
