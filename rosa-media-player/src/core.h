...
protected:
    ...
    void startMplayer( MplayerProcess *player, QString file, double seek = -1 );
    void stopMplayer( MplayerProcess *player );

#ifdef MIDI_SUPPORT
    void startMidiPlayer( WildMidiBackend *player, QString file, double seek = -1 );
    void stopMidiPlayer( WildMidiBackend *player );
#endif

    BackendBase* getBackend( const QString &format );
...
private:
    ...
    QVector<BackendBase*> m_backends;
