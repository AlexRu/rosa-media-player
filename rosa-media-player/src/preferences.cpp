...
void Preferences::reset()
{
...
#ifdef Q_OS_WIN
    mencoder_bin= "mplayer/mencoder.exe";
#else
    mencoder_bin = "mencoder";
#endif

#ifdef Q_OS_WIN
    ffmpeg_bin= "mplayer/ffmpeg.exe"; // ???
#else
    ffmpeg_bin = "ffmpeg";
#endif
...
#ifdef Q_OS_WIN
    wildmidi_bin= "mplayer/wildmidi.exe"; // ???
#else
    wildmidi_bin = "wildmidi";
#endif
#endif
    vo = "";
    ao = "";
...
#else
    QString str = QDesktopServices::storageLocation ( QDesktopServices::PicturesLocation );
    if ( QFile::exists( str ) )
        screenshot_directory = str;
    else
        screenshot_directory = QDir::homePath();
#endif
    ...
    if (QFile::exists("/dev/dvd"))
        dvd_device = "/dev/dvd";
    else
    {
        QString dvdname = "/dev/dvd";
        for (int i = 0; i<10 ;i++ )
        {
            QString str = dvdname + QString::number(i);
            if (QFile::exists(str))
            {
                dvd_device = str;
                break;
    ...
    {
        QString cdname = "/dev/cdrom";
        for (int i = 0; i<10 ;i++ )
        {
            QString str = cdname + QString::number(i);
            if (QFile::exists(str))
            {
                cdrom_device = str;
                break;
    ...
    qDebug() << dvd_device << " " << cdrom_device;
#endif
    ...
    use_pausing_keep_force = false; //true; because not screenshot while pausing
...
void Preferences::save()
{
    ...
    set->setValue("youtube", youtubeEnabled);
...
//    set->setValue("use_extract_audio", use_extract_audio);
...
void Preferences::load()
{
    ...
    set->beginGroup( "general");
    youtubeEnabled = set->value("youtube", youtubeEnabled).toBool();
...
//    use_extract_audio = set->value("use_extract_audio", use_extract_audio).toBool();
...
#if DVDNAV_SUPPORT
    use_dvdnav = true; //set->value("use_dvdnav", use_dvdnav).toBool();
#endif
