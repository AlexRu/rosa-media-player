...
#else
    // If a rosa-media-player.ini exists in the app path, will use that path
    // for the config file by default
    if (QFile::exists( a.applicationDirPath() + "/rosamp.ini" ) )
    {
        config_path = a.applicationDirPath();
        qDebug("main: using existing %s", QString(config_path + "/rosamp.ini").toUtf8().data());
    }
#endif
